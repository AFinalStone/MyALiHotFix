...
#### 2、使用AssetManager对资源进行修复（包括Assets文件夹下的资源和Res文件夹下的资源）
...
#### 3、使用Native的方法对So库进行修复

 So库的修复本质上是对Native方法的修复和替换。
 Native方法的注册分两种：

 - 1>静态注册：一般常见的使用JNI的都属于静态注册方式

 - 2>动态注册：JNI-OnLoad

主要采用类似类修复反射注入方式。把补丁so库的路径插入到nativeLi-braryDirectories数组的最前面，就能够达到加载So库的时候是补丁so库，而不是原来so库的目录。
